...
path = r"C:\My_works\Mancos&Marcellus\Marcellus"
os.chdir(path)
...
###################apply the model#######################
clf = RandomForestClassifier(random_state=0, n_estimators=200, min_samples_split=10,n_jobs=3, max_depth=70,class_weight="balanced")
clf.fit(X_train, Y_train)

##################save the trained model using pickle##########################
#https://www.geeksforgeeks.org/saving-a-machine-learning-model/
#https://stackabuse.com/scikit-learn-save-and-restore-models/
filename = 'RFmodel_14features_new.sav' #C:\My_works\Mancos&Marcellus\Mancos\RF14\RF_14feature_reults
import pickle
...
import joblib
joblib.dump(clf, "./RFmodel_14features_new.joblib")
# load, no need to initialize the loaded_rf
loaded_rf = joblib.load("./RFmodel_14features_new.joblib")
Y3= loaded_rf.predict(X_test)
result3 = loaded_rf.score(X_test, Y_test)
print(result3)
""" 
...
filename = 'RFmodel_14features.sav'
...
#print(confusion_matrix(Y_train,Ypredict1))
print(classification_report(Y_train,Ypredict1))
print(accuracy_score(Y_train, Ypredict1))
##############
...
res = sns.heatmap(confusion_s1_df, annot=True, fmt="d" )
#res.invert_yaxis()
confusion_s1_df.to_csv('confusion_matrix_RF14Features_test_withoutname.csv')

####recreate the segmented images with RF###########
os.makedirs("RF_14feature_reults")
os.chdir(r'RF_14feature_reults') #main folder
Num=21 #number of images 
NumArray = int(len(Ypredict)/Num)
for i in range(0,Num):
    Ypredict1= Ypredict[NumArray*i:(i+1)*NumArray]
    final= np.uint8(Ypredict1.reshape(994,969))#for mancos 994*968 for marcellus 994*969
    #cv2.imwrite(f"K_mean_1dimention/label_reshape{name}.tif", res2)
    cv2.imwrite(f"RF14features_segmentedimage_{i}.tif",final)
