...
path = r"K:\New folder\New_05222021\Marcellus\Final_plot\set1-2"#K:\New folder\New_05222021\Mancos\data #K:\New folder\New_05222021\Marcellus #K:\New folder\New_05222021\Marcellus\Final_plot\set1
os.chdir(path)
...
normalizer = preprocessing.Normalization(input_shape=[1, 14])
...
X_train = np.array(X_train).reshape(X_train.shape[0], 1, 14)
history = model.fit(X_train, Y_train, epochs=100, validation_split = 0.2,
                    callbacks=[call1])
probability_model = tf.keras.Sequential([model, tf.keras.layers.Softmax()])

X_test = np.array(X_test).reshape(X_test.shape[0], 1, 14)
...
########
from tensorflow.python.keras import losses
os.makedirs("saved_model3_FNN14")
model.save_weights(r"saved_model3_FNN14/my_modelMarcellus-weights")
...
model1.load_weights(r"K:\New folder\New_05222021\Marcellus\saved_model3_FNN14\my_modelMarcellus-weights")
...
########################################
########################################
### for providing more images
probability_model1 = tf.keras.Sequential([model1, tf.keras.layers.Softmax()])
X_train = np.array(X_train).reshape(X_train.shape[0], 1, 14)
predictions2 = np.argmax(probability_model1.predict(X_train),axis=2)
os.chdir(r'K:\New folder\New_05222021\Marcellus\Final_plot\set1-2\predict') #main folder #C:\Users\pza0029\Shale_project\Mancos_ziess_test\core1\U-net\images_for_dataAugmentation\Mancos\plot_view\cropped\predict
 #K:\New folder\New_05222021\Mancos\Final_plot\set10\predict

Num=200 #number of images #50 or 100
NumArray = int(len(predictions2)/Num)
for i in range(0,Num):
    predictions3= predictions2[NumArray*i:(i+1)*NumArray]
    final= np.uint8(predictions3.reshape(994,968))#994,968 for mancos . 994,969 for marcellus
    segm1 = (final == 0)
    segm2 = (final == 1)
    segm3 = (final == 2)
    segm4 = (final ==3)
...
# all_segments[segm7]=(6)
    ...
    #cv2.imwrite(f"FNN14_segmentedimage_{i}.tiff",all_segments)
    plt.imsave(f"results_{i}.tiff",all_segments)
...
############Save history
pd.DataFrame.from_dict(history.history).to_csv("FNN_14features.csv")
...
#res.invert_yaxis()
confusion_s1_df.to_csv('confusion_matrix_FNN14Features_withoutname.csv')
...
# ------------------------------------------------------- #
os.mkdir("predicted_images_FNN14")
os.chdir(r'K:\New folder\New_05222021\Marcellus\predicted_images_FNN14') #main folder
...
for i in range(0,Num):
    ...
    #cv2.imwrite(f"K_mean_1dimention/label_reshape{name}.tif", res2)
    cv2.imwrite(f"FNN14_segmentedimage_{i}.tif",final)
