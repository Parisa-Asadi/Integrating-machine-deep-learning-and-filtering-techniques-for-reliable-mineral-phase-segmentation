...
import os
os.chdir(r'C:\Users\pza0029\Shale_project\Mancos_ziess_test\core1\U-net\images_for_dataAugmentation\Mancos') #main folder
#####
#from simple_multi_unet_model import multi_unet_model #Uses softmax #use this if you want to apply auto-tune part other wise use original instead.
...
for directory_path in glob.glob(r"C:\Users\pza0029\Shale_project\Mancos_ziess_test\core1\U-net\images_for_dataAugmentation\Mancos\Train\augmented1\cropped"):
...
train_masks = [] 
for directory_path in glob.glob(r"C:\Users\pza0029\Shale_project\Mancos_ziess_test\core1\U-net\images_for_dataAugmentation\Mancos\Train\labels\augmented1\cropped"):
...
for directory_path in glob.glob(r"C:\Users\pza0029\Shale_project\Mancos_ziess_test\core1\U-net\images_for_dataAugmentation\Mancos\Test\augmented1\cropped"):
    for img_path in glob.glob(os.path.join(directory_path, "*.tif")):
        img = cv2.imread(img_path)       
        #img = cv2.resize(img, (SIZE_Y, SIZE_X))
...
#Capture mask/label info as a list
test_masks = [] 
for directory_path in glob.glob(r"C:\Users\pza0029\Shale_project\Mancos_ziess_test\core1\U-net\images_for_dataAugmentation\Mancos\Test\labels\augmented1\cropped"):
    for mask_path in glob.glob(os.path.join(directory_path, "*.tif")):
        mask = cv2.imread(mask_path, 0)       
        #mask = cv2.resize(mask, (SIZE_Y, SIZE_X), interpolation = cv2.INTER_NEAREST)  #Otherwise ground truth changes due to interpolation
...
for directory_path in glob.glob(r"K:\New folder\New_05222021\Mancos\Final_plot\set1\cropped"): #C:\Users\pza0029\Shale_project\Mancos_ziess_test\core1\U-net\images_for_dataAugmentation\Mancos\plot_view\cropped
    for img_path in glob.glob(os.path.join(directory_path, "*.tif")):
        img = cv2.imread(img_path)       
        #img = cv2.resize(img, (SIZE_Y, SIZE_X))
...
# from sklearn.utils import class_weight
# weight = class_weight.compute_class_weight('balanced', np.unique(train_masks_reshaped_encoded), train_masks_reshaped_encoded)
# weight = {i : weight[i] for i in range(5)} # change the number of classes here is 5
...
stop_early = tensorflow.keras.callbacks.EarlyStopping(monitor="val_loss", mode='min', patience=10)
...
model.save('Mancos_test_fscore_TotalLoss_HyperbandTunning.hdf5')
...
#dice_loss = sm.losses.DiceLoss(class_weights=np.array([0.3, 0.3, 0.05, 0.05,0.3]))
dice_loss = sm.losses.DiceLoss(class_weights=np.array([ 9.79762827 , 1.1264202  , 1.22337743 , 0.31722042 ,24.76786313]))
focal_loss = sm.losses.CategoricalFocalLoss(gamma=2)
total_loss = (1 * dice_loss) + ( 2* focal_loss)
...
model.save('Mancos_Final_fscore_TotalLoss_diceWeighted_dice1_2focal.hdf5')
...
os.chdir(r'K:\New folder\New_05222021\Mancos\Final_plot\set1\predict') #main folder #C:\Users\pza0029\Shale_project\Mancos_ziess_test\core1\U-net\images_for_dataAugmentation\Mancos\plot_view\cropped\predict
...
import sklearn
sklearn.metrics.f1_score(y_test[:,:,:,0], y_pred_argmax, average='macro') #‘micro’, ‘macro’, ‘samples’,’weighted’, ‘binary’} or None, default=’binary’
...
os.chdir(r'C:\Users\pza0029\Shale_project\Mancos_ziess_test\core1\U-net\images_for_dataAugmentation\Marcellus\Test\Y_pred_UNet_metrics1&focal&dice_loss') #main folder
...
os.chdir(r'C:\Users\pza0029\Shale_project\Mancos_ziess_test\core1\U-net\images_for_dataAugmentation\Mancos\predictedImages_UNet\Y_pred_new') #main folder
...
# all_segments[segm7]=(6)
    #predictions2= all_segments
    plt.imsave(f"U_net_{i}.tiff",all_segments)
    #cv2.imwrite("reatached.tiff",cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
#####################################################################

##################################################################### SAVE HISTORY (loss, val)
#df_loss_accuraccy = pd.DataFrame([])
#df_loss_accuraccy.to_csv("df_loss_accuraccy.csv")
pd.DataFrame.from_dict(history.history).to_csv("loss_IOU_F1Score_Mancos.csv")
